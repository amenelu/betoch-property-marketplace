  ...
  return (
    <div className="min-h-screen bg-[#f6f4ee]">
      <aside className="fixed inset-y-0 z-20 hidden w-64 bg-[#173c34] p-6 text-white lg:block">
          ...
          {links.map((x, i) => {
            ...
            return (
              <button
                ...
                className={`flex min-h-11 w-full items-center gap-3 rounded-xl px-3 text-sm ${view === x ? "bg-white text-[#173c34]" : "text-white/70 hover:bg-white/10"}`}
              >
                <Icon size={18} />
                {x}
              </button>
        ...
        <Link href="/" className="absolute bottom-6 text-sm text-white/60">
          ← Back to marketplace
        </Link>
      </aside>
          ...
          {links.map((x, i) => {
            ...
            return (
              <button
                ...
                key={x}
                aria-current={view === x ? "page" : undefined}
                className={`grid min-h-14 min-w-14 place-items-center rounded-xl px-2 text-[10px] ${view === x ? "bg-[#e7eee9] font-bold text-[#173c34]" : "text-stone-500"}`}
              >
      ...
      >
        {sellerInquiries.length ? (
          sellerInquiries.map((x) => (
            ...
            >
              <select
                value={x.status}
                onChange={(e) =>
                  updateInquiry(x.id, e.target.value as typeof x.status)
                }
          ...
          <Empty
            ...
            text="New property inquiries will appear here."
          />
        )}
      </Panel>
    );
  ...
  return (
    <>
      ...
      <Panel
        ...
        action={
          <Link href="/dashboard/new" className="font-bold text-[#d85d3f]">
            Add property
          </Link>
        }
      >
        <SimpleTable
          headers={["Property", "Price", "Status", "Views", "Action"]}
          rows={listings.map((p) => [
            p.title,
            formatPrice(p.price),
...
function AdminContent({ view }: { view: string }) {
  const { reports, updateReport } = useDemo();
  const [listingStatus, setListingStatus] = useState<Record<string, string>>(
    {},
  );
  const [usersState, setUsersState] = useState<Record<string, string>>({});
  const [checks, setChecks] = useState<Record<string, string>>({});
  if (view === "Properties")
    return (
      <Panel
        title="Property moderation"
        intro="Review submitted listings and publication status."
      >
        {properties.map((p, i) => (
          <Row
            key={p.id}
            title={p.title}
            subtitle={`${p.neighborhood} · ${formatPrice(p.price)}`}
            badge={
              listingStatus[p.id] || (i < 2 ? "pending review" : "published")
            }
          >
            <div className="flex gap-2">
              <button
                onClick={() =>
                  setListingStatus((s) => ({ ...s, [p.id]: "published" }))
                }
                className="rounded-lg bg-emerald-100 px-3 py-2 text-xs font-bold text-emerald-800"
              >
                Approve
              </button>
              <button
                ...
                className="rounded-lg bg-red-100 px-3 py-2 text-xs font-bold text-red-800"
              >
                Reject
              </button>
            </div>
  ...
  if (view === "Users")
    return (
      <Panel
        ...
        intro="Search, review and suspend marketplace accounts."
      >
        {[
          "Mekdes Tesfaye · Broker",
          "Daniel Bekele · Owner",
          "Abel Girma · Buyer",
          "Selamawit Girma · Broker",
        ].map((x, i) => (
          <Row
            key={x}
            title={x}
            subtitle={`user${i + 1}@example.et`}
            badge={usersState[x] || "active"}
          >
            <button
              onClick={() =>
                setUsersState((s) => ({
                  ...s,
                  [x]: s[x] === "suspended" ? "active" : "suspended",
                }))
              }
              className="rounded-lg border px-3 py-2 text-xs font-bold"
            >
              {usersState[x] === "suspended" ? "Reactivate" : "Suspend"}
            </button>
          </Row>
        ))}
      </Panel>
  ...
  if (view === "Verification")
    return (
      <Panel
        ...
        intro="Each check is recorded separately. Approval is not a legal ownership guarantee."
      >
        {[
          "Seller identity · Mekdes Tesfaye",
          "Location · Sunlit three-bedroom apartment",
          "Ownership document · Private garden villa",
          "Building document · New-build condominium",
        ].map((x) => (
          <Row
            key={x}
            title={x}
            subtitle="Submitted for manual admin review"
            badge={checks[x] || "pending"}
          >
            <div className="flex gap-2">
              <button
                onClick={() => setChecks((s) => ({ ...s, [x]: "approved" }))}
                className="rounded-lg bg-emerald-100 px-3 py-2 text-xs font-bold text-emerald-800"
              >
                Approve
              </button>
              <button
                onClick={() => setChecks((s) => ({ ...s, [x]: "rejected" }))}
                className="rounded-lg bg-red-100 px-3 py-2 text-xs font-bold text-red-800"
              >
                Reject
              </button>
  ...
  if (view === "Reports")
    return (
      <Panel
        ...
        intro="Investigate user reports and record a resolution."
      >
        {reports.length ? (
          reports.map((x) => (
            <Row
              key={x.id}
              title={x.propertyTitle}
              subtitle={`${x.reason.replaceAll("_", " ")} · ${x.description}`}
              badge={x.status}
            >
              <select
                value={x.status}
                onChange={(e) =>
                  updateReport(x.id, e.target.value as typeof x.status)
                }
                className="rounded-lg border px-3 py-2 text-xs"
              >
                <option>open</option>
          ...
          <Empty
            ...
            text="Reports submitted from property pages will appear here."
          />
        )}
      </Panel>
    );
    ...
    <>
      <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        ...
        <Stat
          ...
          label="Published"
          value="486"
          change="38 pending review"
        />
        <Stat
          icon={ShieldCheck}
          label="Verified"
          value="214"
          change="12 awaiting checks"
        />
        <Stat
          icon={MessageCircle}
          label="Open reports"
          value={String(reports.filter((x) => x.status === "open").length)}
          change="Requires review"
        />
      </div>
      <div className="mt-8 grid gap-6 xl:grid-cols-2">
        <Panel title="Listings awaiting review">
        ...
        <Panel title="Verification queue">
          {[
            "Ownership document · #BTH-1048",
            "Location check · #BTH-1032",
            "Identity document · #USR-488",
          ].map((x) => (
            <Row key={x} title={x} subtitle="Awaiting admin" badge="pending" />
          ))}
        </Panel>
      </div>
    </>
...
function Stat({
  ...
  return (
    <div className="rounded-2xl border bg-white p-5">
      <span className="grid h-10 w-10 place-items-center rounded-xl bg-[#e7eee9] text-[#287864]">
        <Icon size={19} />
      </span>
      <p className="mt-5 text-sm text-stone-500">{label}</p>
      <p className="mt-1 text-3xl font-bold text-[#173c34]">{value}</p>
      <p className="mt-2 text-xs text-stone-500">{change}</p>
    </div>
  );
}
function Panel({
  ...
  return (
    <section className="mt-8 overflow-hidden rounded-2xl border bg-white">
      <div className="flex items-center justify-between border-b p-5">
        <div>
          <h2 className="font-serif text-xl font-semibold text-[#173c34]">
            {title}
          </h2>
          {intro ? (
            <p className="mt-1 text-xs text-stone-500">{intro}</p>
          ) : null}
        </div>
        {action}
      </div>
...
function Row({
  ...
  return (
    <div className="flex flex-col gap-3 border-b py-4 last:border-0 sm:flex-row sm:items-center">
      <div className="min-w-0 flex-1">
        <p className="font-semibold">{title}</p>
        <p className="truncate text-xs text-stone-500">{subtitle}</p>
      </div>
      <span className="w-fit rounded-full bg-stone-100 px-2.5 py-1 text-[11px] font-bold capitalize">
        {badge}
      </span>
      {children}
    </div>
      ...
      <table className="w-full min-w-[650px] text-left text-sm">
        <thead>
          <tr>
            {headers.map((x) => (
              <th
                className="border-b px-3 py-3 text-xs uppercase text-stone-500"
                key={x}
              >
                {x}
              </th>
        ...
        <tbody>
          {rows.map((row, i) => (
            <tr key={i}>
              {row.map((x, j) => (
                <td className="border-b px-3 py-4" key={j}>
                  {linkLast && j === row.length - 1 ? (
                    <Link
                      href={`/dashboard/properties/${linkIds?.[i] || properties[i]?.id}/edit`}
                      className="font-bold text-[#d85d3f]"
                    >
                      {x}
                    </Link>
                  ) : (
...
function Empty({ title, text }: { title: string; text: string }) {
  return (
    <div className="py-12 text-center">
      <Clock3 className="mx-auto text-stone-400" />
      <p className="mt-3 font-serif text-xl text-[#173c34]">{title}</p>
